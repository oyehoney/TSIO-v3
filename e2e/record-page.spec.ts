/**
 * record-page.spec.ts — Playwright e2e tests for Innovation Record page (/records/:id).
 *
 * Tests cover all must_haves from 11-PLAN.md:
 * - Breadcrumb "← Back to Catalog" renders and links to /catalog
 * - Record header shows title, maturity badge, review status badge, community badge, owner
 * - Perspective Toggle (role=tablist) always visible with Executive View and Technical View tabs
 * - Default view is Executive View (aria-selected="true" on executive tab)
 * - Clicking Technical View tab switches content, updates URL to ?view=technical
 * - Loading ?view=technical directly opens Technical View
 * - Technical View sections: WHAT WAS EXPLORED, TECHNICAL DETAILS, SECURITY FINDINGS
 * - "Security review NOT completed" warning visible when security_findings is null
 * - Trust & Limitations section visible in both views
 ...
 * - "View Technical Details →" crosslink in executive view
 * - "View Executive Summary →" crosslink in technical view
 * - /records/{nonexistent-id} shows 404 page with "not found" message
 *
 * Strategy: Tests seed a PUBLISHED EXPERIMENT_POC COMMUNITY record via the test-seed API
 * (POST /api/v1/test-seed/published-record), run assertions against the rendered EJS page,
 * then clean up with DELETE /api/v1/test-seed/records/:id.
 *
 * The test-seed API is only registered when NODE_ENV !== 'production' (T-11-07).
 * playwright.config.ts starts the server with NODE_ENV=test.
 */
...
test.beforeAll(async ({ request }) => {
  ...
  // Test-seed endpoint is gated on NODE_ENV !== 'production' (T-11-07).
  const seedRes = await request.post('/api/v1/test-seed/published-record', {
    data: {
      ...
      problem_statement:
        ...
        'to prevent accidental recording of attorney-client communications and sidebars.',
      what_was_explored:
      ...
      outcome_summary:
        ...
        'conditions but Azure Government Cloud network segmentation constraints prevent production deployment.',
      ...
      executive_perspective_text:
        'This effort validated that GPU/CPU audio separation is technically feasible ' +
        'but faces meaningful constraints in the Azure Government Cloud environment.',
      executive_recommendation:
        'This effort is at Proof of Concept stage and is not recommended for production ' +
        'adoption without additional security review.',
      technical_perspective_text:
        'GPU-based audio separation using Python + TensorFlow on Azure GPU VMs. ' +
        'Dependency on premium GPU tiers not available in all Azure Government Cloud regions.',
      security_findings: null,
      performance_findings:
        'Latency exceeds acceptable thresholds for real-time proceedings under standard ' +
        'Azure Government Cloud network conditions.',
      reuse_guidance:
        'Courts without dedicated GPU infrastructure would require hardware provisioning.',
      key_findings: [
      ...
      artifact_links: [
        {
          ...
          url: 'https://sharepoint.ao.dcn/sites/TSIO/AudioSecurityPOC',
...
test.afterAll(async ({ request }) => {
  if (publishedRecordId) {
    // Clean up seeded record — bypasses lifecycle for test teardown
...
test('record header shows title, maturity badge, review status badge, and owner', async ({ page }) => {
  ...
  // Maturity badge (from partial maturity-badge.ejs using data-testid)
  await expect(page.locator('[data-testid="maturity-badge"]')).toBeVisible();
  // Review status badge
  await expect(page.locator('[data-testid="review-status-badge"]')).toBeVisible();
...
test('clicking Technical View tab switches view and updates URL ?view=technical', async ({ page }) => {
  ...
  // URL updated (client-side JS pushes ?view=technical)
  ...
  // Technical sections visible (panels shown/hidden by client-side JS)
...
test('trust disclaimers include COMMUNITY disclaimer for COMMUNITY source_type', async ({ page }) => {
  ...
  // Text includes "team outside I&R"
...
test('trust disclaimers section appears in both executive and technical views', async ({ page }) => {
  await page.goto(`/records/${publishedRecordId}`);
  // Executive view — trust section visible
  await expect(page.getByRole('heading', { name: /TRUST.*LIMITATIONS/i })).toBeVisible();
  // Switch to technical view
  await page.getByRole('tab', { name: 'Technical View' }).click();
  // Trust section still visible (rendered outside perspective panels)
  await expect(page.getByRole('heading', { name: /TRUST.*LIMITATIONS/i })).toBeVisible();
});
