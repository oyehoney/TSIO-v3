...
export const OpportunitySubmissionForm: React.FC<OpportunitySubmissionFormProps> = ({ onSuccess }) => {
  ...
  // Error summary only shown after a submit attempt (not on individual field blur)
  const hasErrorSummary = submitAttempted && (Object.values(errors).some(Boolean) || serverErrors?.length);
