...
test.describe('Share Innovation Form (/share-innovation)', () => {
  ...
  test('Artifact URL with non-https:// value shows inline error', async ({ page }) => {
    ...
    await urlInput.blur();
    // Check the inline validation error element (role="alert") specifically
    await expect(page.getByRole('alert').filter({ hasText: /must begin with https/i })).toBeVisible();
  });

  test('"+ Add another artifact URL" button reveals URL 2 field', async ({ page }) => {
    await page.goto(`${BASE_URL}/share-innovation`);
    // Initially only URL 1 visible — check by looking for the input with label "Artifact URL 2"
    // (getByLabel can match aria-labels on buttons, so we check input count explicitly)
    await expect(page.locator('input[id="artifact_url_1"]')).toHaveCount(0);
    // Click add button
    await page.getByRole('button', { name: /Add another artifact URL/i }).click();
    // URL 2 (index 1) input should now be visible
    await expect(page.locator('input[id="artifact_url_1"]')).toBeVisible();
  });
