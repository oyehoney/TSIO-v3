 ...
 *
 * Note: Canonical implementation also lives at src/admin/pages/EngagementActivityPage.tsx.
...
// ── Inline status popover (per Screen-10 UX) ──────────────────────────────────
...
export const EngagementActivityPage: React.FC = () => {
  ...
  const fetchRoutingEmail = useCallback(async () => {
    try {
      ...
      // Non-critical; silently ignore — routing email display is secondary
    }
  }, []);
  ...
  return (
    <div style={{ padding: '0' }}>
      {/* Page header — "N total · M in the last 7 days" per Screen-10 */}
      ...
      {/* Filter bar — record dropdown, type dropdown, date range select */}
      ...
      {/* Requests table — columns: DATE, TYPE, RECORD, REQUESTOR, STATUS per Screen-10 */}
      {!loading && !error && requests.length > 0 && (
        <>
              ...
              <tbody>
                {requests.map((req, idx) => (
                  ...
                  >
                    ...
                    {/* Inline [Update] action — COMPLETED rows show [View] instead */}
      ...
      {/* Routing email display — below table per Screen-10 */}
      <div
        style={{
          padding: '16px 20px',
