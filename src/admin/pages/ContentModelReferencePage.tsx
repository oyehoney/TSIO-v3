 ...
 * No edit controls — explicitly read-only.
 *
 * API (optional — fails gracefully):
...
// "Definitions require a code change" — this is by design (Screen-12 mockup).
...
export const ContentModelReferencePage: React.FC = () => {
  // API data — falls back to hard-coded if unavailable (501 or any error)
  ...
  useEffect(() => {
    // Fetch maturity-reference and review-status-reference in parallel.
    // If either returns 501 NOT_IMPLEMENTED or any error, use hard-coded constants above.
    // This is correct per Screen-12: "Definitions require a code change" — they ARE compile-time constants.
    Promise.all([
      ...
      // If API returns data, it would be set here.
      // Currently hard-coded fallback is always used (canonical values match).
