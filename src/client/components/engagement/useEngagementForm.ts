/**
 * useEngagementForm.ts — Engagement request form state, validation, and API submission.
 *
 * Manages form state, field-level validation, CAPTCHA token state, API submission to
...
export function useEngagementForm(
  ...
  // Keep a ref to current fields so handleSubmit can read latest values without stale closure
  const fieldsRef = useRef<FormFields>(fields);
  fieldsRef.current = fields;

  // Keep a ref to current captchaToken so handleSubmit reads the latest value
  const captchaTokenRef = useRef<string | null>(captchaToken);
  ...
  const handleBlur = useCallback((field: keyof FormFields) => {
    const value = fieldsRef.current[field] ?? '';
  ...
  const handleSubmit = useCallback(async (recordTitle: string) => {
    ...
    type RequiredField = keyof FormErrors;
    ...
    for (const field of requiredFields) {
      const error = validateField(field, currentFields[field] ?? '');
    ...
    try {
      const response = await fetch('/api/v1/engagement-requests', {
        ...
        body: JSON.stringify({
          ...
          record_id: recordId,
          requestor_name: currentFields.requestorName.trim(),
          requestor_email: currentFields.requestorEmail.trim(),
          requestor_office: currentFields.requestorOffice.trim(),
          description_of_interest: currentFields.descriptionOfInterest.trim(),
          desired_next_step: currentFields.desiredNextStep.trim() || undefined,
