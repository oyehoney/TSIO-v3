/**
 * EngagementRequestModal.tsx — Engagement request modal overlay.
 *
 * Main modal component for submitting an engagement request from the RecordPage Next-Action panel.
 * Mounts as a portal overlay on /records/{id} — does NOT create a new route.
 *
 ...
 * - CAPTCHA via CaptchaWidget (dev-bypass when no site key; real reCAPTCHA v2 in production)
 * - Submit button disabled while submitting or CAPTCHA not yet obtained
 * - Submission error banner at form top (rate limit, server error)
 * - Switches to EngagementConfirmation on successful submission
 * - Cancel button and × button both call onClose
 *
 * Security:
 * - T-13-01: CAPTCHA required before submit (defense-in-depth; server also validates)
 * - T-13-02: All text fields rendered as React children — no dangerouslySetInnerHTML
 * - T-13-04: isSubmitting flag prevents double-submit
 */

import React, { useEffect, useRef } from 'react';
import { useEngagementForm } from './useEngagementForm';
...
export function EngagementRequestModal({
  ...
  useEffect(() => {
    if (isOpen && firstInputRef.current) {
      // Small delay to ensure modal is rendered in DOM before focusing
  ...
  // Keyboard trap: close on Escape, trap Tab within modal (WCAG 2.1 AA)
  useEffect(() => {
  ...
  return (
    // Overlay backdrop — clicking outside modal panel closes it
    <div
      aria-hidden={false}
      style={{
        position: 'fixed',
      ...
      onClick={(e) => {
        // Close only if clicking the backdrop directly (not the modal panel)
    ...
    >
      ...
      >
        ...
        {/* Success confirmation state — replaces form on successful submission */}
        {confirmationState ? (
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              void handleSubmit(recordTitle);
            }}
            noValidate
          >
            {/* Record reference — read-only, pre-populated from RecordPage */}
            <p
              id="engagement-modal-desc"
              style={{ color: '#374151', marginBottom: '20px', fontSize: '15px' }}
            >
            ...
            {/* Submission error banner (rate limit, server error, CAPTCHA failure) */}
            {submitError && (
              <div
                role="alert"
                style={{
            ...
            {/* Describe your interest (required, 20-2000 chars, live character count) */}
            <div style={{ marginBottom: '16px' }}>
              <label
                htmlFor="descriptionOfInterest"
                style={{ display: 'block', fontWeight: 600, marginBottom: '4px' }}
              >
              ...
              <textarea
                ...
                aria-describedby={
                  errors.descriptionOfInterest
                    ? 'descriptionOfInterest-error descriptionOfInterest-count'
                    : 'descriptionOfInterest-count'
                }
                style={{
                  width: '100%',
                  padding: '8px 12px',
              ...
              >
                {errors.descriptionOfInterest ? (
                  <p
                    id="descriptionOfInterest-error"
                    role="alert"
                    style={{ color: '#DC2626', fontSize: '13px', margin: 0 }}
                  >
                    {errors.descriptionOfInterest}
                  </p>
                ) : (
                ...
                <span
                  ...
                  style={{
                    fontSize: '13px',
                    color: descCharCount > 1900 ? '#DC2626' : '#6B7280',
                  }}
                >
                  {descCharCount} / 2000
                </span>
            ...
            {/* CAPTCHA widget — submit disabled until completed */}
            <div style={{ marginBottom: '20px' }}>
              <CaptchaWidget
